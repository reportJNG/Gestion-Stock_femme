  ...
  return useQuery({
    ...
    queryFn: async () => {
      let request = supabase
        .from('sales')
        .select('id, sale_number, total_ttc, customer_name, created_at, sale_items(id)', { count: 'exact' })
        .order('created_at', { ascending: false })
        .range(page * PAGE_SIZE, page * PAGE_SIZE + PAGE_SIZE - 1);

      // ✅ FILTER: Only show sales from the current user
      if (user?.id) {
        request = request.eq('sold_by', user.id);
      }
...
export function useSaleDetail(saleId: string) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // ✅ Check if this sale belongs to the current user
